...
export default function SignUpForm(){
    ...
    // create a state for the hook
    const [verifying, setVerifying] = useState(false);

    // handle all error hooks
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [authError, setAuthError] = useState<string | null>(null);

    // handle verification code and errors
    ...
    const handleVerificationSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if(result.status === "complete"){
                ...
                router.push("/dashboard"); // navigate the user to dashboard after signup successful
            } else {
            ...
            setVerificationError(
                error.errors?.[0]?.message || "An erroroccured during the signup. Please try again"
            )
        } finally {
            setIsSubmitting(false);
        }
                ...
                <CardBody className="py-6">
                   ...
                   <form onSubmit={handleVerificationSubmit} className="space-y-6">
                        <div className="space-y-2">
                            <label htmlFor="verificatioCode" className="text-sm font-medium text-default-900">
                                Verification Code
                            </label>
                            <Input
                                id="verificationCode"
                   ...
                   <div className="mt-6 text-center">
                        <p className="text-sm text-default-500">
                            ...
                            className="text-primary hover:underline font-mdeium"
                            >
                                Resend code
                            </button>
                        </p>
